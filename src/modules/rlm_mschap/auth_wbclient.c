		     ...
		     uint8_t nthashhash[NT_DIGEST_LENGTH])
{
	 ...
	 */
	len = radius_xlat(user_name, sizeof(user_name), inst->ntlm_username, request, NULL);
	if (len < 0) goto done;

	authparams.account_name = user_name;

	if (inst->ntlm_domain) {
		len = radius_xlat(domain_name, sizeof(domain_name), inst->ntlm_domain, request, NULL);
		if (len < 0) goto done;
	...
	case WBC_ERR_WINBIND_NOT_AVAILABLE:
		RDEBUG("Check that winbind is running and that FreeRADIUS");
		RDEBUG("has permission to connect to the winbind socket!");
		break;
	case WBC_ERR_DOMAIN_NOT_FOUND:
		RDEBUG("domain not found");
		break;
